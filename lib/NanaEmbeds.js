    ...
    getInfoEmbed(id, msg) {
        return new Promise(async (fulfill, reject) => {
            try {
                ...
                embed.setFooter(
                    `React with 📖 to continue reading / ${res.id}`
                );
                ...
                if (info.category[0])
                    embed.addField("Categories", info.category, true);
                embed.addField("Pages", res.num_pages, true);
                if (info.tag[0])
                    embed.addField(
                        "Tags",
                        info.tag[0] ? info.tag.join(", ") : info.tag
                    );
        ...
        return new Promise(async (fulfill, reject) => {
            try {
                ...
                res.images.pages.forEach((page, i) => {
                    doujin.push(
                        `https://i.nhentai.net/galleries/${res.media_id}/${
                            i + 1
                        }.${TYPE[page.t]}`
                    );
                });
                ...
                forwards.on("collect", async (f) => {
                    ...
                    read.setFooter(
                        `Page ${pagination} of ${doujin.length} / ${res.id}`
                    );
                ...
                favorit.on("collect", async (f) => {
                    ...
                    if (favorite.includes(res.id)) {
                        this.client.favorite.deleteUserFavoritID(
                            msg.author.id,
                            res.id
                        );
                        return msg.channel
                        ...
                        this.client.favorite.setUserFavoritID(
                            msg.author.id,
                            res.id
                        );
                        return msg.channel
        ...
        return new Promise(async (fulfill, reject) => {
            try {
                ...
                const forwardsTenFilter = (reaction, user) =>
                    reaction.emoji.name === "⏩" && user.id === msg.author.id;
                const backwardsTen = r.createReactionCollector(
                    backwardsTenFilter
                );
                ...
                const forwardsTen = r.createReactionCollector(
                    forwardsTenFilter
                );
